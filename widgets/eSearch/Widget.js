...
define([
  ...
  function (
    ...
    LoadingShelter, ioquery, SpatialReference, registry, WidgetManager
    ) { /*jshint unused: false*/
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _initTabContainer: function () {
        ...
        this.cbxAddTolerance.setValue(this.config.addpointtolerancechecked || false);
        this.cbxMultiGraphic.setValue(this.config.multipartgraphicsearchchecked || false);
      ...
      search: function (geometry, layerIndex, /* optional */expressIndex, theValue, spatialRelationship) {
        ...
        if (geometry) {
          ...
          query.where = this.lastWhere = where;
        ...
        var featureLayer = new FeatureLayer(layerConfig.url, {mode: FeatureLayer.MODE_SNAPSHOT});
        ...
        if (layerConfig.definitionexpression){
          if (geometry) {
            ...
            featureLayer.setDefinitionExpression(layerConfig.definitionexpression + ' AND ' + this.lastWhere);
          }
        } else{
          if(!geometry){
            featureLayer.setDefinitionExpression(this.lastWhere);
          }
        }
        on(featureLayer, 'load', lang.hitch(this, function(){
          featureLayer.queryFeatures(query, lang.hitch(this, this._onSearchFinish, layerIndex, featureLayer), lang.hitch(this, this._onSearchError));
        }));
      },
      ...
      clear: function (/* optional */closeAtt) {
        ...
        this._clearLayers();
        if(closeAtt){
          console.info(this.selTab, this.isSelTabVisible());
...
//todo consider adding back the required flag
      ...
      _onSearchFinish: function (layerIndex, featureLayer, results) {
        var layerConfig = this.config.layers[layerIndex];
        var currentLayer;
        var showAttachments = false;
        if('showattachments' in layerConfig && layerConfig.showattachments){
        ...
        if (len === 0) {
          ...
          this.divResultMessage.textContent = this.divResultMessage.innerText = this.nls.featuresSelected + results.features.length;
        }
        for (var i = 0; i < len; i++) {
          ...
          for (var att in featureAttributes) {
            ...
            if(this.initiator && (this.initiator === 'graphic' || this.limitMapExtentCbx.getValue())){
              if(fld.name === objectIdField){
                this.oidArray.push(featureAttributes[att]);
              }
            }
            ...
            if (titlefield && upperCaseFieldName === titlefield.toUpperCase()) {
              ...
              if(this._isVisible(att, layerIndex)){
                content = content + br + "<em>" + this._getAlias(att, layerIndex) + "</em>: " + value;
                br = "<br>";
              }
            }
          }
      ...
      _drawResults: function (layerIndex, results, currentLayer) {
        ...
        var features = results.features,
            ...
            centerpoint;
        if(layerConfig.layersymbolfrom === 'config'){
        ...
        if(this.initiator && layerConfig.shareResult && (this.initiator === 'graphic' || this.limitMapExtentCbx.getValue())){
          this.lastWhere = layerConfig.objectIdField + ' IN (' + this.oidArray.join() + ')';
          if(layerConfig.definitionexpression){
            currentLayer.setDefinitionExpression(layerConfig.definitionexpression + ' AND ' + this.lastWhere);
          }else{
            currentLayer.setDefinitionExpression(this.lastWhere);
          }
        }
        currentLayer.minScale = 0;
        currentLayer.maxScale = 0;
        currentLayer.effectiveMinScale = 0;
        currentLayer.effectiveMaxScale = 0;
        currentLayer.defaultVisibility = true;
        currentLayer.name = this.resultLayers[layerIndex].name;
